...
snapshot() {
    ...
    # Удаление ресурса перенумеровывает id остальных, а они инлайнятся в байткод как
    # константы. Нормализованная копия маскирует операнды — остаётся структура кода.
